...
const I18N = (function(){
  ...
  const dict = {
    ar: {
      ...
      integration_deploy_hint: "لو البيانات مش بتوصل للشيت رغم الرابط صحيح: افتح مشروع Apps Script → Deploy → Manage deployments → عدّل النشر الحالي → تأكد إن \"Who has access\" = Anyone → ولازم تعمل \"New version\" في كل مرة تغيّر فيها كود Code.gs عشان التعديل يتفعّل فعليًا.",
    ...
    en: {
      ...
      integration_deploy_hint: "If data still isn't reaching the Sheet even though the URL is correct: open the Apps Script project → Deploy → Manage deployments → edit the current deployment → make sure \"Who has access\" = Anyone → and you must create a \"New version\" every time you change Code.gs for the change to actually take effect.",
